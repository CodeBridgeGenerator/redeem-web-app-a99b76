...
const CartItemHistoryCreateDialogComponent = (props) => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      props.alert({
        type: "success",
        title: "Create info",
        message: "Info Cart Item History created successfully",
      });
      ...
      props.alert({
        type: "error",
        title: "Create",
        message: "Failed to create in Cart Item History",
      });
    }
    setLoading(false);
  };
  ...
  return (
    <Dialog
      header="Create Cart Item History"
      visible={props.show}
      closable={false}
      onHide={props.onHide}
      modal
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <Calendar
              id="completedDate"
              value={
                _entity?.completedDate ? new Date(_entity?.completedDate) : null
              }
              onChange={(e) => setValByKey("completedDate", e.value)}
              showTime
              hourFormat="24"
            />
          </span>
